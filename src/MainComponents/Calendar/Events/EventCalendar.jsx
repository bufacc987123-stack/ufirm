...
class EventCalendar extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            filterEventType: 'E',
            filterCategoryList: [], setSelectedOptionsCategory: [], filterCategroy: 'NULL',
            filterAssigneeList: [], setSelectedOptionsAssignee: [], filterAssignee: 'NULL',
            detailspageHtmlFormatDesc: '',
            taskStatusList: [], filterEventStatus: '',
        };
        ...
        switch (event.status) {
            case "Completed":
                text = <span style={{ color: "black" }}>
                    <i className='fa fa-check' title='Completed' style={{ fontSize: 12, marginRight: "4px" }}></i>
                    {`${event.eventNumber}- ${event.title}`}
                </span>
                break;
            case "Overdue":
                text = <span style={{ color: "black" }}>
                    <i className="fa fa-exclamation-triangle" title='Overdue' style={{ fontSize: 12, marginRight: "4px" }}></i>
                    {`${event.eventNumber}- ${event.title}`}
                </span>
                break;
            case "Scheduled":
                text = <span style={{ color: "black" }}>
                    <i className="fa fa-clock" title='Scheduled' style={{ fontSize: 12, marginRight: "4px" }}></i>
                    {`${event.eventNumber}- ${event.title}`}
                </span>
                break;
            case "Deleted":
                text = <span style={{ color: "black" }}>
                    <i className="fa fa-trash" title='Deleted' style={{ fontSize: 12, marginRight: "4px" }}></i>
                    {`${event.eventNumber}- ${event.title}`}
                </span>
                break;
            default:
        ...
        switch (type) {
            ...
            case 'E':
                model.push({
                    ...
                    "EventType": this.state.filterEventType ? this.state.filterEventType : "NULL",
                    "Categories": this.state.filterCategroy ? this.state.filterCategroy : "NULL",
                    "Assignees": this.state.filterAssignee ? this.state.filterAssignee : "NULL",
                    "EventStatus": this.state.filterEventStatus ? this.state.filterEventStatus : "NULL"
                });
                break;
            case 'DT':
                model.push({
                        ...
                        switch (type) {
                            case 'R':
                                ...
                                rData.forEach(element => {
                                    newCategory.push({ Id: element.catId, Name: element.name, value: element.name, label: element.name, color: '#0052CC' });
                                });
    ...
    getAsignee() {
        this.ApiProviderr.manageAssigne(parseInt(this.props.PropertyId)).then(resp => {
            if (resp.ok && resp.status == 200) {
                return resp.json().then(rData => {
                    ...
                    let newAssignee = [];
                    rData.forEach(element => {
                        newAssignee.push({ Id: element.emailAddress, Name: element.displayName, value: element.displayName, label: element.displayName, color: '#0052CC' });
                    });
                            ...
                            case 'DT':
                                if (rData != null) {
                                    ...
                                    if (rData.isAllday) {
                                        ...
                                        rData.startTime = new Date(Date.parse(`${rData.startDate} ${rData.startTime}`))
                                        rData.endTime = new Date(Date.parse(`${rData.endDate} ${rData.endTime}`))
                                        rData.startDate = new Date(Date.parse(rData.startDate));
                                        rData.endDate = new Date(Date.parse(rData.endDate));
                                    }
                                    ...
                                    singleEvtData = rData;
                                    // find assignee id 
                                    let currAssignee = this.state.assigneeList.filter((x) => x.emailAddress === singleEvtData.assignToEmail);
                                    ...
                                    EventDetailsModelInstance.setCategoryId(singleEvtData.categoryId);
                                    ...
                                    EventDetailsModelInstance.setNotificationBefore(singleEvtData.notificationBefore);
                                    EventDetailsModelInstance.setRepeat(singleEvtData.repeat);
                                    EventDetailsModelInstance.setAssignee(singleEvtData.assigneeId);
                                    EventDetailsModelInstance.setRepeateEndBy(singleEvtData.repeateEndBy);
                                    EventDetailsModelInstance.setSubEventId(singleEvtData.subEventId);
                                    EventDetailsModelInstance.setCompletedBy(singleEvtData.completedBy);
                                    EventDetailsModelInstance.setEventNumber(singleEvtData.eventNumber);
                                    EventDetailsModelInstance.setIsDeleteRequest(singleEvtData.isDeleteRequest);
                                    EventDetailsModelInstance.setStatusRequest(singleEvtData.status);
                                    this.setState({ EventDetailsModelInstance: EventDetailsModelInstance, showDetailsModal: true, detailspageHtmlFormatDesc: singleEvtData.description }
                                        , () => {
    ...
    eventAssigneeFilter(value, event) {
        this.setState({ setSelectedOptionsAssignee: value }, () => {
            let data = this.state.setSelectedOptionsAssignee.
    ...
    eventCategoryFilter(value, event) {
        this.setState({ setSelectedOptionsCategory: value }, () => {
            let data = this.state.setSelectedOptionsCategory.
                map((item) => { return item.Id })
                                    ...
                                    <ul className="nav ml-auto tableFilterContainer">
                                        ...
                                        <li className="nav-item">
                                            <select
                                                ...
                                                onChange={(e) => { this.eventStatusFilter(e.target.value) }}>
                                                <option value="NULL">Filter Event Status</option>
                                                {
                                                    this.state.taskStatusList ? this.state.taskStatusList.map((e, key) => {
                                                        return <option key={key} value={e.statusName}>{e.statusName}</option>;
                                                    })
                                                        : null
                                                }
                                            </select>
                                        </li>
                                        <li className="nav-item" style={{ width: "300px", zIndex: "999" }}>
                                            <MultiSelectInline
...
export default connect(mapStoreToprops, mapDispatchToProps)(EventCalendar);
